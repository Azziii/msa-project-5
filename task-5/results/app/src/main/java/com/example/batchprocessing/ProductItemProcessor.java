...
public class ProductItemProcessor implements ItemProcessor<Product, Product> {
	...
	public Product process(final Product product) {
		...
		AtomicReference<Product> transformedProduct =
				new AtomicReference<>(
						new Product(
								productId,
								productSku,
		...
		String sql =
				"SELECT * FROM loyality_data WHERE productSku=" + productSku;
		...
		jdbcTemplate.query(
				...
				.map(loyalityData ->
						new Product(
								...
								loyalityData))
				.map(productResult -> {
					transformedProduct.set(productResult);
					return null;
				});
